...
//                                    sh "bundle exec parallel_cucumber features -n $params.threads -f json_pretty --out cucumber.json -o \"-t @buyerTableAddBuyerStatus env=$params.env sys=$params.system jobExecutionPlatform=jenkins -f json --retry 1\"  "
                                    sh "bundle exec parallel_cucumber features -n $params.threads -f json_pretty --out cucumber.json -t @buyerTableAddBuyerStatus -o \"env=$params.env sys=$params.system jobExecutionPlatform=jenkins --retry 1\"  "
// | tee test-output.log
